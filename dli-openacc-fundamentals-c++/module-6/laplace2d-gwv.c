...
{
    double error = 0.0;
    #pragma acc parallel loop num_gangs( 64 ) num_workers( 32 ) vector_length( 32 ) reduction(max:error) present(A,Anew)
    for( int j = 1; j < n-1; j++)
    {
...
{
    #pragma acc parallel loop num_gangs( 64 ) num_workers( 32 ) vector_length( 32 ) present(A,Anew)
    for( int j = 1; j < n-1; j++)
    {
        #pragma acc loop vector
        for( int i = 1; i < m-1; i++ )
