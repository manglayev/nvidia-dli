...
void initializeOnHost(float num, float *host_c, int N)
{
  for(int i = 0; i < N; ++i)
  {
    host_c[i] = num;
...
int main()
{
  ...
  if(addVectorsErr != cudaSuccess) printf("Error 1: %s\n", cudaGetErrorString(addVectorsErr));

  asyncErr = cudaDeviceSynchronize();
  if(asyncErr != cudaSuccess) printf("Error 2: %s\n", cudaGetErrorString(asyncErr));
