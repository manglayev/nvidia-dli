...
__global__ void step_kernel_mod(float* temp_in, float* temp_out)
{
...
void step_kernel_ref(float* temp_in, float* temp_out)
{
  int i00, im10, ip10, i0m1, i0p1;
  float d2tdx2, d2tdy2;
...
int main()
{
  ...
  */
  ...
  for (istep=0; istep < nstep; istep++)
  {
    ...
    step_kernel_ref(temp1_ref, temp2_ref);
  ...
  //dim3 threads_per_block(128, 256, 1);
  //dim3 number_of_blocks((ni*nj/threads_per_block.x)+1, (ni*nj/threads_per_block.y)+1, 1);
  //my mistake was in counting the number of threads and blocks
  //good approximation conversion of ni and ni (100 and 200) to 32 x (100/32+1) = 128 and 16 x (200/16+1) = 208
  dim3 threads_per_block(32, 16, 1);
  dim3 number_of_blocks((nj/threads_per_block.x)+1, (ni/threads_per_block.y)+1, 1);
  // Execute the modified version using same data
  for (istep=0; istep < nstep; istep++)
  {
    step_kernel_mod<<<number_of_blocks, threads_per_block>>>(temp1, temp2);
